...
- **Status in `eak/`:** ◐ **inc 1–7 built** — `PowerDomain` (seam + `erc-power-balance` rule,
  ...
  [ADR-0026](../docs/decisions/0026-band-b-signal-flow.md)), `Contract`+`Interface` (seam +
  `erc-interface-contract` rule — the logical electrical meaning above raw connectivity per §32;
  [ADR-0027](../docs/decisions/0027-band-b-interface-contract.md)), and `Bus` (seam +
...
| 5 Band B — logical-electrical | L5 | ◐ **inc 1–7 built** (`PowerDomain` + `ClockDomain` + `ReturnPath` + `PinCapability`/`PinAssignment` + `Signal` + `Contract`/`Interface` + `Bus` + `erc-power-balance`/`erc-clock-domain-conflict`/`erc-return-path-required`/`erc-pin-mux-conflict`/`erc-pin-capability`/`erc-signal-driver-sink`/`erc-interface-contract`/`erc-bus-topology`; 382 tests; ADR-0022–0028) |
